from workflows import (
    workflow, wait, conv_append, conv_read, conv_replace_with, Latest,
)
...
def summarize():
    ...
    yield conv_append(role='assistant', content='Python was created by Guido van Rossum...')
    yield conv_append(role='user', content='What about version 3?')

    # Summarize the first 3 messages
    yield conv_replace_with(
        [{'role': 'system', 'content': 'Summary: User asked about Python and its history.'}],
        start_ref=r1,
        end_ref=r3,
    )
