...
class TestWaitAll:
    def test_waits_for_both(self, env):
        ...
        finished_results = sorted(
            e.payload.result
            for e in inbox
            if isinstance(e.payload, WorkflowFinished)
    ...
    def test_all_children_finish_before_result(self, env):
        ...
        assert state.finished
        assert (
            state.workflows[state.root_workflow_id].result == 2 + 1
        )  # counter(2)=2, adder(2)=1
...
class TestSleep:
    ...
    def test_sleep_with_concurrent_children(self, env):
        ...
        engine.step(
            store, eid, now=0.0
        )  # parent: sleep(10), wake_at=10. child: yields 0
