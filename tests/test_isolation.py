...
def _init_git_repo(path: Path, branch="main"):
    ...
    subprocess.run(
        ["git", "init", "-b", branch], cwd=str(path), capture_output=True, check=True
    )
    subprocess.run(
...
class TestDockerIsolation:
    ...
    def test_network_none(self, tmp_path):
        ...
        result = docker.run_shell(
            tmp_path, "wget -q -O- http://example.com 2>&1 || echo FAIL"
        )
...
class TestStorageSame:
    def test_same_returns_parent_dir(self, tmp_path):
        ...
        child_dir, branches = setup_child_workspace(
            parent, tmp_path / "child", None, config
        )
...
class TestStorageBranch:
    ...
    def test_branch_shares_directory(self, tmp_path):
        ...
        (parent / "file.txt").write_text("hello")
        subprocess.run(
            ["git", "add", "."], cwd=str(parent), capture_output=True, check=True
        )
        subprocess.run(
        ...
        child_dir, _ = setup_child_workspace(
            parent, tmp_path / "unused", {".": "main"}, config
        )
        assert (child_dir / "file.txt").read_text() == "hello"
