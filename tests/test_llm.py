...
import os
import pytest
from workflows import workflow, conv_append, conv_read, Latest, Engine, EngineConfig, Store
from workflows.ops import ai

has_anthropic = bool(os.environ.get('ANTHROPIC_API_KEY'))
has_openai = bool(os.environ.get('OPENAI_API_KEY'))

requires_anthropic = pytest.mark.skipif(not has_anthropic, reason='ANTHROPIC_API_KEY not set')
...
class TestOpenAILlm:
    def test_simple_question(self):
        store = Store(':memory:')
        engine = Engine(EngineConfig(workflows_registry=REGISTRY))
        eid = engine.start(store, 'ask_openai', ['What is 2+2? Reply with just the number.'])
        state = run_to_completion(engine, store, eid)
        assert state.finished
