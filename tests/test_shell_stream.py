...
from workflows import (
    workflow, Engine, EngineConfig, Store,
    shell_stream_start, shell_stream_next, ShellStreamLine,
)
...
@workflow
def stream_echo():
    stream = yield shell_stream_start('echo "line1"; echo "line2"; echo "line3"', isolation=HostIsolation())
    lines = []
...
@workflow
def stream_stderr():
    stream = yield shell_stream_start('echo out1; echo err1 >&2; echo out2; echo err2 >&2', isolation=HostIsolation())
    stderr_lines = []
    line = yield shell_stream_next(stream)
    while not line.finished:
