...
def answer_prompts(store, eid, engine, answers: list[str]):
    ...
    answered_ids = {e.payload.request_id for e in inbox if isinstance(e.payload, UserPromptResult)}
    unanswered = [e for e in outbox
...
def run_to_completion(engine, store, eid, answer_sequence: list[str], max_steps=50):
    ...
    for _ in range(max_steps):
        ...
        if unanswered and answers:
            ...
            store.append_event(
                eid, event.workflow_id, 'inbox',
                UserPromptResult(request_id=event.payload.request_id, response=answers.pop(0)),
            )
