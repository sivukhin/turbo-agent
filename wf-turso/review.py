...
import json
import os
from workflows import workflow, conv_append, shell, shell_stream_start, shell_stream_next
...
def review(pr_number):
    ...
    yield conv_append(role='assistant', content=f'Starting review of PR #{pr_number}...')
    ...
    if clone.exit_code != 0:
        yield conv_append(role='assistant', content=f'Clone failed:\n```\n{clone.stderr}\n```')
        return 'clone failed'

    checkout = yield shell(f'cd turso && gh pr checkout {pr_number}', isolation=HostIsolation())
    if checkout.exit_code != 0:
        yield conv_append(role='assistant', content=f'Checkout failed:\n```\n{checkout.stderr}\n```')
        return 'checkout failed'

    yield conv_append(role='assistant', content='Building tursodb...')
    build = yield shell('cd turso && cargo build --bin tursodb', isolation=HostIsolation())
    if build.exit_code != 0:
        yield conv_append(role='assistant', content=f'Build failed:\n```\n{build.stderr[:2000]}\n```')
        return 'build failed'
    ...
    while True:
        ...
        for line in raw.stdout:
            ...
            if etype == 'system':
                if subtype == 'init':
                    yield conv_append(role='assistant',
            ...
            # assistant messages: contain tool_use or text content
                ...
                for block in content_blocks:
                    if block.get('type') == 'text':
                        ...
                        if text.strip():
                            final_text = text
                            yield conv_append(role='assistant', content=text,
                                              meta={'labels': 'hidden'})
                    elif block.get('type') == 'tool_use':
                        yield conv_append(role='tool_use',
                                          content={
                                              'id': block.get('id', ''),
                                              'name': block.get('name', '?'),
            ...
            # user messages: tool results
                ...
                for block in content_blocks:
                    if block.get('type') == 'tool_result':
                        ...
                        yield conv_append(role='tool_result',
                                          content={
