...
class DurableGenerator:
    ...
    def checkpoint(self):
        ...
        return {
            ...
            'yv': loc.get('__yv__'),
            'locals': {k: v for k, v in loc.items()
...
def workflow(func):
    ...
    orig_varnames = [v for v in func.__code__.co_varnames
                     if v not in ('__checkpoint__',)]

    yield_count = sum(1 for i in bc
                      if isinstance(i, Instr) and i.name == 'YIELD_VALUE')
    ...
    def _make_gen(*args, checkpoint=None):
        raw = types.FunctionType(new_code, {
            **func.__globals__,
