...
class Store:
    ...
    def get_created_at(self, execution_id: str) -> float:
        cur = self.conn.cursor()
        cur.execute(
            "SELECT created_at FROM executions WHERE execution_id = ?", (execution_id,)
        )
        row = cur.fetchone()
        return row[0] if row else 0.0

    def append_event(
        self, execution_id: str, workflow_id: str | None, category: str, payload
    ) -> None:
    ...
    def _read_events(
        self, execution_id: str, category: str, after_event_id: int = 0
    ) -> list[Event]:
        cur = self.conn.cursor()
        cur.execute(
    ...
    def read_all_events(
        self, execution_id: str, after_event_id: int = 0
    ) -> list[Event]:
    ...
    def list_executions(self) -> list[tuple[str, ExecutionState, float]]:
        cur = self.conn.cursor()
        cur.execute(
            "SELECT execution_id, state, created_at FROM executions ORDER BY execution_id"
        )
    ...
    def _list_layer_messages(
        ...
        for (
            ...
            refs.append(
                MessageRef(conversation_id, msg_id, layer, role, meta, event_time or 0)
            )
    ...
    def conv_replace_with(
        ...
        for i, msg in enumerate(new_messages):
            ...
            new_refs.append(
                MessageRef(
                    conversation_id, msg_id, new_layer, msg["role"], meta, event_time
                )
            )
